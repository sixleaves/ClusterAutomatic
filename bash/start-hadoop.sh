...
ssh atguigu@$CLUSTER_PREFIX_HOSTNAME$NAMENODE_HOST "source /etc/profile&&${start_dfs_cmd}"
ssh atguigu@$CLUSTER_PREFIX_HOSTNAME$RESOURCE_HOST "source /etc/profile&&${start_rs_cmd}"
ssh atguigu@$CLUSTER_PREFIX_HOSTNAME$HISTORY_SERVER "source /etc/profile&&${start_hs_cmd}"
