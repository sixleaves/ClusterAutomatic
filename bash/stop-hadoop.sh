...
ssh $USER_NAME@$CLUSTER_PREFIX_HOSTNAME$NAMENODE_HOST "source /etc/profile&&${stop_dfs_cmd}"

echo -e "--------------\033[32m$CLUSTER_PREFIX_HOSTNAME$RESOURCE_HOST\033[0m---------------"
ssh $USER_NAME@$CLUSTER_PREFIX_HOSTNAME$RESOURCE_HOST "source /etc/profile&&${stop_rs_cmd}"

echo -e "--------------\033[32m$CLUSTER_PREFIX_HOSTNAME$HISTORY_SERVER\033[0m---------------"
ssh $USER_NAME@$CLUSTER_PREFIX_HOSTNAME$HISTORY_SERVER "source /etc/profile&&${stop_hs_cmd}"
